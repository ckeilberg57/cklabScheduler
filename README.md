# Whiteglove Scheduler (Known as sbalkcScheduler in reference below)
...
Enabled by default. User records are stored in the SQLite database with PBKDF2-SHA256 hashed passwords (minimum 12 characters). The installer creates the initial admin account during setup.
...
See [docs/ENTRA_AUTHENTICATION.md](docs/ENTRA_AUTHENTICATION.md) for the full Azure registration walkthrough.

### Roles

| Role | Access |
|---|---|
| `administrator` | Full access — meeting management, config |
| `scheduler_user` | Standard access — create and manage meetings |
...
| `LOCAL_AUTH_ENABLED` | `true`/`false` — enable local admin account authentication |
| `ENTRA_ENABLED` | `true`/`false` — enable Microsoft Entra ID authentication |
| `SESSION_COOKIE_SECURE` | `true` in production; `false` for local HTTP dev only |
